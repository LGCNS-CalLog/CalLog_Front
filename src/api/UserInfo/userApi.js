...
//   try {
//     const response = await apiClient.post("/userStatus/update", {
//       height,
//       weight,
//     });
//     if (
...
//       console.error("정보 수정 요청 실패:", errorMessage);
...
export const updateUserInfo = async ({ height, weight }) => {
  try {
    ...
    if (
      ...
      return { height, weight }; // 요청에 사용한 값 리턴
    } else {
      throw new Error(
        response.data.message ||
          "정보 수정 요청에 실패했습니다. 잠시 후 다시 시도해주세요."
    ...
    throw new Error(
      error.response?.data?.message || "정보 수정 요청에 실패했습니다."
    );
  }
};
